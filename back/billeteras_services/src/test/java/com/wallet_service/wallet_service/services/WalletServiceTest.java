...
public class WalletServiceTest {
    ...
    void testObtenerPorId_Existe() {
        // Arrange: Simulamos que la billetera con ID 1 sí existe en la BD
        ...
        // Assert: Verificamos que traiga los datos correctos (USD)
    ...
    void testObtenerPorId_NoExiste() {
        // Arrange: Simulamos que buscar el ID 99 no devuelve nada de la BD
        when(walletRepository.findById(99)).thenReturn(Optional.empty());

        // Act: Intentamos buscar ese ID inexistente
        WalletDto resultado = walletService.obtenerPorId(99);

        // Assert: Validamos que la respuesta sea nula
